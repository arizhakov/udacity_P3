...
def make_pipeline():
    ...
    pipeline = [{"$match":{"country":"India"}},
                {"$unwind":"$isPartOf"},
                {"$project":{"region":"$isPartOf","name":"$name","pop":"$population"}}, # Calculate your answer by first finding the average population of cities in each region
                {"$group": {"_id":"$region",
                            "avg_ofRegion":{"$avg":"$pop"}}},
                { "$group" : {"_id" : "India Regional City Population Average", #then, by calculating the average of the regional averages
                              "avg":{"$avg":"$avg_ofRegion"}}}  
                ]
    return pipeline

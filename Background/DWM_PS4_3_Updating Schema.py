    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            if (line['binomialAuthority_label'] != "NULL"):   
                data[strip_parans(line['rdf-schema#label'])] = line['binomialAuthority_label']
    print data
    return data
...
def update_db(data, db):
    # YOUR CODE HERE
    ### there is a subtlety here, check lectures. Prof said something about mistakes can delete entire db!
    for i in data.keys():
        db.arachnid.update({'label':i},
                           {"$set" : {"classification.binomialAuthority" : data[i]} }
                           )

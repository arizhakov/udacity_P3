...
def article_overview(kind, period):
    ...
    for i in range(len(data)):
        ...
        # - list of dictionaries, where the dictionary key is "section" and value is "title"
        new_dict = {}
        new_dict[data[i]['section']] = data[i]['title'] 
        titles.append(new_dict)
        
        # second Req
        # - list of URLs for all media entries with "format": "Standard Thumbnail"
        for j in range(len(data[i]['media'])):
            for k in range(len(data[i]['media'][j]['media-metadata'])):
                if (data[i]['media'][j]['media-metadata'][k]['format'] == "Standard Thumbnail"):
                    urls.append(data[i]['media'][j]['media-metadata'][k]['url'])

    #print data                    
    #print data[2]['title']
    #print data[19]['section']
    #print data['section']
    
    #print data[0]['media'][0]['media-metadata'][0]['format'] == "Standard Thumbnail"
    #print data[0]['media'][0]['media-metadata'][0]['url']

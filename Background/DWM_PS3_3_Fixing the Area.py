...
def fix_area(area):
    ...
    if area == "NULL":
        ...
        pos_pipe = area.find("|")
        str_num_1 = area[1:pos_pipe]
        str_num_2 = area[pos_pipe+1:-1]
        if len(str_num_1) > len(str_num_2):
            area = float(str_num_1)
        elif len(str_num_2) > len(str_num_1):
            area = float(str_num_2)
        else:
            area = None
    else:
        try:

...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for attr in element.attrib:
            ...
            #elif element.tag.tag == "tag":
            #    if not problemchars.search(element.get('k')):
            #        node['address'] = {1}
            else:
        ...
        for child in element:
            ...
            if child.tag == "tag":
                ...
                print "child.attrib['k']: ", child.attrib['k']
                #print "problemchars.search(child.attrib['k']):  ", problemchars.search(child.attrib['k'])
                
                if (problemchars.search(child.attrib['k']) == None):
                    if (child.attrib['k'][:5]=="addr:" and not (re.search(":", child.attrib['k'][5:]))):
                        print "inside if one..."
                        #print "ADDRESS!"
                        print "child.attrib['k'][5:] =  ", child.attrib['k'][5:]
                        print "child.attrib['v'] =  ", child.attrib['v']
                        node['address'][child.attrib['k'][5:]] = child.attrib['v']
                        print "node['address'][child.attrib['k'][5:]] =  ",node['address'][child.attrib['k'][5:]]
                    elif (child.attrib['k'][:5]!="addr:"):
                        print "inside if two..."                    
                        node[child.attrib['k']] = child.attrib['v']
                        #
                    #print "first 5", child.attrib['k'][:5]
                    #if (child.attrib['k'][:]):
                        #
                    #
                #
                #if child.attrib == "k":
                #    print "CHILDREN!; k tag"
                #    if not problemchars.search(element.get('k')):
                #        node['address'] = {1}
            if child.tag == "nd":
                print "node refs"
